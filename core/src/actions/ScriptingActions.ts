...
export class SetVariableAction implements ActionExecutor {
  ...
  constructor() {
    this.definition = createActionDefinition({
      ...
      outputs: [{ name: 'value', type: DataType.ANY, description: 'The value that was set' }],
      version: '1.0.0'
    });
  }
  ...
  async execute(inputs: Record<string, any>, context: IWorkflowContext): Promise<ActionResult> {
    ...
    context.setVariable(name, value, DataType.ANY, 'local');
...
export class GetVariableAction implements ActionExecutor {
  ...
  constructor() {
    this.definition = createActionDefinition({
      ...
      outputs: [{ name: 'value', type: DataType.ANY, description: 'The variable value' }],
      version: '1.0.0'
    });
  }
  ...
  async execute(inputs: Record<string, any>, context: IWorkflowContext): Promise<ActionResult> {
    ...
    if (value === undefined) {
      return { success: false, error: `Variable '${name}' not found`, recoverable: true };
    }
    return { success: true, outputs: { value } };
  }
}
