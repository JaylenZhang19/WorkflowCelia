...
export class GetLatestPhotosAction implements ActionExecutor {
  ...
  constructor() {
    this.definition = createActionDefinition({
      ...
      outputs: [{ name: 'photos', type: DataType.IMAGE, isArray: true, description: 'Array of photo objects' }],
      version: '1.0.0'
    });
  }
...
export class SaveToAlbumAction implements ActionExecutor {
  ...
  constructor() {
    this.definition = createActionDefinition({
      ...
      category: 'photos',
      inputs: [
        { name: 'media', type: DataType.IMAGE, isArray: true, required: true, description: 'Media to save' },
        { name: 'albumName', type: DataType.TEXT, required: false, default: 'Recents', description: 'Album name' }
      ],
      outputs: [{ name: 'savedCount', type: DataType.NUMBER, description: 'Number of items saved' }],
      version: '1.0.0'
    });
  }
